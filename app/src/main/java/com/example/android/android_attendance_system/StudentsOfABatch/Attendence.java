...
public class Attendence extends AppCompatActivity{
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        //Log.i("pppppp",monthYearForPdf.substring(3,10));

        //for(int h=0;h<s.size();h++){
            //Log.i("wwwww","n"+classAttendence[h]);
        //}
    }
    ...
    public void getbatchpdf(long batchId){
        ...
        File root = new File(Environment.getExternalStorageDirectory(), batchName+"("+branchName+")))");
        if (!root.exists()) {
            root.mkdirs();   // create root directory in sdcard
        }
        File pdffile = new File(root,batchSub+"..for.."+monthYearForPdf.substring(3,10)+".pdf");
        ...
        try {
            ...
            doc.open();
            doc.add(new Paragraph("Batch: "+batchName+"\n"));
            doc.add(new Paragraph("Branch: "+branchName+"("+batchSem+")"+"\n"));
            doc.add(new Paragraph("Sub: "+batchSub));
            doc.add(new Paragraph("Month/Year: "+monthYearForPdf+"\n\n\n"));
